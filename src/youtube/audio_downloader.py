...
from src.youtube.utils.clean_url import clean_youtube_url
from src.youtube.utils.create_folder import create_folder
from src.youtube.utils.on_progress import on_progress
from src.youtube.utils.output_filename import output_filename
    ...
    try:
        ...
        for dynamic_stream in best_streams:
            ...
            if audio_stream:
                audio_stream.download(
                    output_path=download_folder, filename=output_filename(yt.title)
                )
                return True
        ...
        if error_type in error_messages:
            ...
            messagebox.showerror(
                title="Error", message=f"An unknown error occurred: {e}"
            )
        return False
...
def audio_downloader(audio_url_list: List[str], download_folder: str) -> None:
    ...
    for url in audio_url_list:
        ...
        try:
            ...
            if _download_audio(url=cleaned_url, download_folder=download_folder):
            ...
            logger.error(
                f"Error has occurred while downloading the audio from {url}: {e}"
            )
